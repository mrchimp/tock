function Tock(options){this.go=false;this.interval=options.interval||10;this.countdown=options.countdown||false;this.final_time=0;if(options.callback!==undefined){this.callback=options.callback;}else{this.callback=function(){};}
if(options.complete!==undefined){this.complete=options.complete;}else{this.complete=function(){};}}
Tock.prototype.reset=function(){this.stop();this.start_time=0;this.time=0;this.elapsed='0.0';};Tock.prototype.start=function(time){if(this.countdown){this._startCountdown(time);}else{this._startTimer();}};Tock.prototype.stop=function(){this.go=false;this.final_time=(Date.now()-this.start_time);window.clearTimeout(this.timeout);};Tock.prototype.lap=function(){if(this.go){var now;if(this.countdown){now=this.duration_ms-(Date.now()-this.start_time);}else{now=(Date.now()-this.start_time);}
return now;}
return this.final_time;};Tock.prototype.msToTime=function(ms){var milliseconds=ms%1000,seconds=Math.floor((ms/1000)%60).toString(),minutes=Math.floor((ms/(60*1000))%60).toString();if(seconds.length===1){seconds='0'+seconds;}
if(minutes.length===1){minutes='0'+minutes;}
return minutes+":"+seconds+":"+milliseconds;};Tock.prototype.timeToMS=function(time){var time_split=time.split(':'),ms=parseInt(time_split[0],10)*60000;if(time_split.length){ms+=parseInt(time_split[1],10)*1000;}
